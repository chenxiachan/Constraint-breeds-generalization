...
class SimpleANN(nn.Module):
    """
    Standard 3-layer ANN.
    Matches SNN architecture for fair comparison.
    """
    ...
    def forward(self, x):
        # Handle temporal input by taking mean
        if len(x.shape) == 3:
            x = x.mean(dim=1)
...
def run_single_experiment(agent_type, delta=None, run_id=0, train_episodes=500,
                          eval_seed=None, verbose=False, early_stopping=False):
...
def run_comprehensive_experiments(num_runs=5, train_episodes=500, use_eval_seed=True):
    ...
    # --- Incremental checkpointing (added for NeurIPS rebuttal re-run): ---
    # per-(delta,run) seeds are deterministic, so resuming from a completed
    # run boundary reproduces exactly the same results as an uninterrupted run.
    import pickle
    _ckpt_path = os.path.join('./experiment_outputs', 'checkpoint_results.pkl')
    _start_run = 0
    if os.path.exists(_ckpt_path):
        with open(_ckpt_path, 'rb') as _f:
            all_results = pickle.load(_f)
        _start_run = min(len(v) for v in all_results.values())
        print(f"\n[checkpoint] Found checkpoint with {_start_run} complete runs "
              f"-> resuming from run {_start_run + 1}")

    for run_id in range(_start_run, num_runs):
        ...
        print(f"  [checkpoint] saved ({run_id + 1} runs complete)")
